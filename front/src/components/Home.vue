...
window.addEventListener("load", () => {
  gsap
    ...
    .to(".background", { scale: 1.7, z: 400, transformOrigin: "center center", ease: "power1.inOut" })
...
<template>
  ...
  <div class="wrapper">
    ...
    <div class="image-container">
      <img class="background" :src="Hole" alt="image" />
    </div>
  </div>
</template>
...
.gradient-fade-bot {
  ...
  margin-top: -300px;
  position: relative;
  background: linear-gradient(to bottom, rgba(0, 0, 0, 0), #1F2E38);
}
.gradient-fade-top {
  height: 300px;
  position: relative;
  background: linear-gradient(to top, rgba(0, 0, 0, 0), #1F2E38);
}
